...
# 5. Symlinks the 'Virat-Code' CLI command into ~/.local/bin
...
echo ""
echo -e "${CYAN}⚕ Virat Code Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink Virat-Code into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up Virat-Code command..."

VIRAT_CODE_BIN="$SCRIPT_DIR/venv/bin/virat-code"
mkdir -p "$HOME/.local/bin"
ln -sf "$VIRAT_CODE_BIN" "$HOME/.local/bin/virat-code"
echo -e "${GREEN}✓${NC} Symlinked Virat-Code → ~/.local/bin/virat-code"
...
# ============================================================================
# Seed bundled skills into ~/.virat-code/skills/
# ============================================================================

VIRAT_CODE_SKILLS_DIR="${VIRAT_CODE_HOME:-$HOME/.virat-code}/skills"
mkdir -p "$VIRAT_CODE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.virat-code/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$VIRAT_CODE_SKILLS_DIR/" 2>/dev/null || true
...
echo "     Virat-Code setup"
echo ""
echo "  3. Start chatting:"
echo "     virat-code"
echo ""
echo "Other commands:"
echo "  Virat-Code status        # Check configuration"
echo "  Virat-Code gateway install # Install gateway service (messaging + cron)"
echo "  Virat-Code cron list     # View scheduled jobs"
echo "  Virat-Code doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m virat_code_cli.main setup
fi
