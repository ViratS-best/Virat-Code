...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `virat-code skills browse` (labeled "official") and install it with `virat-code skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Virat Sisodiya Discord](https://discord.gg/NousResearch). Users can install it with `virat-code skills install`.
...
```bash
mkdir -p ~/.virat-code/{cron,sessions,logs,memories,skills}
...
│
├── skills/                   # Bundled skills (copied to ~/.virat-code/skills/ on install)
...
### User configuration (stored in `~/.virat-code/`)

| Path | Purpose |
|------|---------|
| `~/.virat-code/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.virat-code/.env` | API keys and secrets |
| `~/.virat-code/auth.json` | OAuth credentials (Nous Portal) |
| `~/.virat-code/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.virat-code/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.virat-code/state.db` | SQLite session database |
| `~/.virat-code/sessions/` | JSON session logs |
| `~/.virat-code/cron/` | Scheduled job data |
| `~/.virat-code/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`virat_code_state.py`) with full-text search and unique session titles. JSON logs go to `~/.virat-code/sessions/`.
...
Create `~/.virat-code/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `virat_code_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `virat_code_cli/skin_engine.py` for the full schema and existing skins as examples.
...
2. **Test manually**: Run `Virat-Code` and exercise the code path you changed
