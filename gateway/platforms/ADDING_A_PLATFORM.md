...
```python
"virat-code-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _VIRAT_CODE_CORE_TOOLS,
    "includes": []
},
```

And add it to the `virat-code-gateway` composite:

```python
"virat-code-gateway": {
    "includes": [..., "virat-code-your-platform"]
}
```
...
## 12. Status Display (`virat_code_cli/status.py`)
...
## 13. Gateway Setup Wizard (`virat_code_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ virat_code_cli/ toolsets.py \
  --include="*.py" -l | sort -u
