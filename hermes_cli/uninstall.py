...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing Virat Code or hermes PATH entries
            new_lines = []
            skip_next = False
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'Virat Code' in content:
                    wrapper.unlink()
...
def run_uninstall(args):
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.hermes/, just remove the Virat Code subdir
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/Virat Code/main/scripts/install.sh | bash", Colors.DIM))
        print()
