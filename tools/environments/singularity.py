...
_SNAPSHOT_STORE = Path.home() / ".virat-code" / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    Resolution order:
      ...
      4. ~/.virat-code/sandboxes/singularity (fallback)
    """
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "Virat-Code") / "Virat-Code"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"virat_code_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "virat-code-overlays"
