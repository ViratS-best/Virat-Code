...
Architecture:
  1. Parent generates a `virat_code_tools.py` stub module with RPC functions
...
# ---------------------------------------------------------------------------
# virat_code_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_virat_code_tools_module(enabled_tools: List[str]) -> str:
    """
    Build the source code for the virat_code_tools.py stub module.
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["VIRAT_CODE_RPC_SOCKET"])
...
def execute_code(
    ...
    # --- Set up temp directory with virat_code_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="virat_code_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"virat_code_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated virat_code_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_virat_code_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "virat_code_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["VIRAT_CODE_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("VIRAT_CODE_TIMEZONE", "").strip()
        if _tz_name:
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``virat-code tools`` (e.g. web is turned off),
    ...
    description = (
        ...
        f"Available via `from virat_code_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Limits: 5-minute timeout, 50KB stdout cap, max 50 tool calls per script. "
        "terminal() is foreground-only (no background or pty).\n\n"
        "Print your final result to stdout. Use Python stdlib (json, re, math, csv, "
        "datetime, collections, etc.) for processing between tool calls.\n\n"
        "Also available (no import needed — built into virat_code_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from virat_code_tools import {import_str}` "
