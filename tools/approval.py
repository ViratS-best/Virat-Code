...
DANGEROUS_PATTERNS = [
    ...
    (r'\btee\b.*(/etc/|/dev/sd|\.ssh/|\.virat-code/\.env)', "overwrite system file via tee"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    session_key = os.getenv("VIRAT_CODE_SESSION_KEY", "default")
    if is_approved(session_key, pattern_key):
        return {"approved": True, "message": None}

    is_cli = os.getenv("VIRAT_CODE_INTERACTIVE")
    is_gateway = os.getenv("VIRAT_CODE_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("VIRAT_CODE_EXEC_ASK"):
