...
Configuration is loaded from ~/.virat-code/config.yaml under the 'tts:' key.
...
DEFAULT_OUTPUT_DIR = os.path.expanduser("~/.virat-code/audio_cache")
MAX_TEXT_LENGTH = 4000


# ===========================================================================
# Config loader -- reads tts: section from ~/.virat-code/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.virat-code/config.yaml.
    ...
    try:
        ...
        logger.debug("virat_code_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.virat-code/config.yaml (tts: section).
    ...
    platform = os.getenv("VIRAT_CODE_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.virat-code/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
