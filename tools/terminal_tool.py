...
def _check_disk_usage_warning():
    ...
    try:
        # Get total size of Virat-Code directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "virat-code-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("VIRAT_CODE_GATEWAY_SESSION")
    ...
    for failure in sudo_failures:
        if failure in output:
            return output + "\n\n💡 Tip: To enable sudo over messaging, add SUDO_PASSWORD to ~/.virat-code/.env on the agent machine."
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (VIRAT_CODE_INTERACTIVE=1).
    ...
    try:
        os.environ["VIRAT_CODE_SPINNER_PAUSE"] = "1"
        ...
        if "VIRAT_CODE_SPINNER_PAUSE" in os.environ:
            del os.environ["VIRAT_CODE_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str) -> tuple[str, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (VIRAT_CODE_INTERACTIVE=1):
    ...
    if not sudo_password:
        ...
        if os.getenv("VIRAT_CODE_INTERACTIVE"):
...
def get_active_environments_info() -> Dict[str, Any]:
    ...
    for task_id in _active_environments.keys():
        scratch_dir = _get_scratch_dir()
        pattern = f"virat-code-*{task_id[:8]}*"
        import glob
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            session_key = os.getenv("VIRAT_CODE_SESSION_KEY", "")
            ...
            try:
                ...
                if check_interval and background:
                    ...
                    process_registry.pending_watchers.append({
                        ...
                        "chat_id": os.getenv("VIRAT_CODE_SESSION_CHAT_ID", ""),
                    })
...
if __name__ == "__main__":
    ...
    print(f"  TERMINAL_SANDBOX_DIR: {os.getenv('TERMINAL_SANDBOX_DIR', '~/.virat-code/sandboxes')}")
