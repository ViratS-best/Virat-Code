...
def load_virat_code_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.virat-code/config.yaml if not provided)
    ...
    """
    # Load config from ~/.virat-code/config.yaml
    config = load_virat_code_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.virat-code/.env")
            else:
