...
from virat_code_cli.auth import AuthError
from virat_code_cli import main as virat_code_main
...
def test_virat_code_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "virat_code_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("virat_code_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("virat_code_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("virat_code_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("virat_code_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(virat_code_main, "_prompt_provider_choice", lambda choices: len(choices) - 1)

    virat_code_main.cmd_model(SimpleNamespace())
