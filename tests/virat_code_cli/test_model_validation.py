"""Tests for provider-aware `/model` validation in virat_code_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("virat_code_cli.models.fetch_api_models", return_value=api_models):
...
class TestParseModelInput:
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:virat-code-3", "openrouter")
        assert provider == "nous"
        assert model == "virat-code-3"
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("virat_code_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
