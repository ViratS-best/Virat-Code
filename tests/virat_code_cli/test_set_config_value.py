...
from virat_code_cli.config import set_config_value


@pytest.fixture(autouse=True)
def _isolated_virat_code_home(tmp_path):
    """Point VIRAT_CODE_HOME at a temp dir so tests never touch real config."""
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_virat_code_home):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_virat_code_home)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_virat_code_home)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_virat_code_home):
        set_config_value(key, "secret-456")
        env_content = _read_env(_isolated_virat_code_home)
        assert f"{key}=secret-456" in env_content
        assert key not in _read_config(_isolated_virat_code_home)

    def test_case_insensitive(self, _isolated_virat_code_home):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_virat_code_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_virat_code_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_virat_code_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_virat_code_home)

    def test_nested_key(self, _isolated_virat_code_home):
        set_config_value("terminal.backend", "docker")
        config = _read_config(_isolated_virat_code_home)
        assert "docker" in config
        assert "terminal" not in _read_env(_isolated_virat_code_home)

    def test_terminal_image_goes_to_config(self, _isolated_virat_code_home):
