...
from virat_code_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials


def _setup_nous_auth(
    virat_code_home: Path,
    *,
    ...
    (virat_code_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    _setup_nous_auth(virat_code_home, refresh_token="refresh-old")
    ...
    monkeypatch.setattr("virat_code_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("virat_code_cli.auth._mint_agent_key", _fake_mint_agent_key)
