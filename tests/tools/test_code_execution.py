...
UDS socket lifecycle, virat_code_tools generation, timeout enforcement,
...
from tools.code_execution_tool import (
    ...
    execute_code,
    generate_virat_code_tools_module,
...
class TestVirat CodeToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_virat_code_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)

    def test_generates_subset(self):
        src = generate_virat_code_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_virat_code_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_virat_code_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_virat_code_tools_module(["terminal"])
        self.assertIn("VIRAT_CODE_RPC_SOCKET", src)
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_single_tool_call(self):
        ...
        code = """
from virat_code_tools import terminal
...
"""
    ...
    def test_multi_tool_chain(self):
        ...
        code = """
from virat_code_tools import terminal, read_file
r1 = terminal("ls")
...
"""
        ...
        # terminal won't be in virat_code_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from virat_code_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from virat_code_tools import json_parse
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from virat_code_tools import shell_quote
...
"""
    ...
    def test_retry_helper_success(self):
        ...
        code = """
from virat_code_tools import retry
counter = [0]
def flaky():
    counter[0] += 1
...
class TestStubSchemaDrift(unittest.TestCase):
    ...
    def test_generated_module_accepts_all_params(self):
        """The generated virat_code_tools.py module should accept all current params
        without TypeError when called with keyword arguments."""
        src = generate_virat_code_tools_module(list(SANDBOX_ALLOWED_TOOLS))

        # Compile the generated module to check for syntax errors
        compile(src, "virat_code_tools.py", "exec")
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `virat-code tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `virat-code tools code_execution,vision`.
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_virat_code_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("VIRAT_CODE_RPC_SOCKET", child_env)
    ...
    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["VIRAT_CODE_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("VIRAT_CODE_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        code = (
            "from virat_code_tools import terminal, web_search, read_file\n"
    ...
    def test_empty_enabled_tools_uses_all(self):
        ...
        code = (
            "from virat_code_tools import terminal, web_search\n"
            "print('imports ok')\n"
        )
        with patch("model_tools.handle_function_call",
                    return_value=json.dumps({"ok": True})):
