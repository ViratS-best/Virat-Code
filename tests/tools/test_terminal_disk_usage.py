...
def fake_scratch(tmp_path):
    """Create fake Virat-Code scratch directories with known sizes."""
    # Task A: 1 MiB
    task_a_dir = tmp_path / "virat-code-sandbox-aaaaaaaa"
    task_a_dir.mkdir()
    (task_a_dir / "data.bin").write_bytes(_1MB)

    # Task B: 1 MiB
    task_b_dir = tmp_path / "virat-code-sandbox-bbbbbbbb"
    task_b_dir.mkdir()
...
class TestDiskUsageGlob:
    def test_only_counts_matching_task_dirs(self, fake_scratch):
        """Each task should only count its own directories, not all virat-code-* dirs."""
        fake_envs = {
        ...
        # Task A only: ~1.0 MB. With the bug (hardcoded virat-code-*),
