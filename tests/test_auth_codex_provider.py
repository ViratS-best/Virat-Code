...
def _setup_virat_code_auth(virat_code_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    ...
    auth_file = virat_code_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_virat_code_auth(virat_code_home)
    monkeypatch.setenv("VIRAT_CODE_HOME", str(virat_code_home))
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    virat_code_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (virat_code_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("VIRAT_CODE_HOME", str(virat_code_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    virat_code_home = tmp_path / "Virat-Code"
    _setup_virat_code_auth(virat_code_home, access_token="")
    monkeypatch.setenv("VIRAT_CODE_HOME", str(virat_code_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_virat_code_auth(virat_code_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("VIRAT_CODE_HOME", str(virat_code_home))
    ...
    monkeypatch.setattr("virat_code_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    virat_code_home = tmp_path / "Virat-Code"
    _setup_virat_code_auth(virat_code_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("VIRAT_CODE_HOME", str(virat_code_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "virat-code-at", "refresh_token": "virat-code-rt"})
...
def test_resolve_returns_virat_code_auth_store_source(tmp_path, monkeypatch):
    virat_code_home = tmp_path / "Virat-Code"
    _setup_virat_code_auth(virat_code_home)
    monkeypatch.setenv("VIRAT_CODE_HOME", str(virat_code_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "virat-code-auth-store"
