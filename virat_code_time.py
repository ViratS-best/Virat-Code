"""
Timezone-aware clock for Virat-Code.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``VIRAT_CODE_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.virat-code/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Virat-Code never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Virat-Code requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("VIRAT_CODE_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
