...
Usage:
    ...
    Virat-Code chat                # Interactive chat
    Virat-Code gateway             # Run gateway in foreground
    Virat-Code gateway start       # Start gateway as service
    Virat-Code gateway stop        # Stop gateway service
    Virat-Code gateway status      # Show gateway status
    Virat-Code gateway install     # Install gateway service
    Virat-Code gateway uninstall   # Uninstall gateway service
    Virat-Code setup               # Interactive setup wizard
    Virat-Code logout              # Clear stored authentication
    Virat-Code status              # Show status of all components
    Virat-Code cron                # Manage cron jobs
    Virat-Code cron list           # List cron jobs
...
# Load .env from ~/.virat-code/.env first, then project root as dev fallback
from dotenv import load_dotenv
from virat_code_cli.config import get_env_path, get_virat_code_home
...
# Point mini-swe-agent at ~/.virat-code/ so it shares our config
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(get_virat_code_home()))
os.environ.setdefault("MSWEA_SILENT_STARTUP", "1")

import logging

from virat_code_cli import __version__
from virat_code_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from virat_code_cli.config import get_env_path, get_virat_code_home
    from virat_code_cli.auth import get_auth_status

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from virat_code_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_virat_code_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'virat-code sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'Virat-Code setup' at any time to configure.")
        sys.exit(1)
...
def cmd_gateway(args):
    ...
    from virat_code_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from virat_code_cli.config import get_env_value, save_env_value
    ...
    session_dir = Path.home() / ".virat-code" / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: Virat-Code gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        print("⚠ Pairing may not have completed. Run 'Virat-Code whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from virat_code_cli.setup import run_setup_wizard
    run_setup_wizard(args)


def cmd_model(args):
    """Select default model — starts with provider selection, then model picker."""
    from virat_code_cli.auth import (
    ...
    from virat_code_cli.config import load_config, save_config, get_env_value, save_env_value
    ...
    effective_provider = (
        os.getenv("VIRAT_CODE_INFERENCE_PROVIDER")
        or config_provider
        or "auto"
    )
    try:
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from virat_code_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from virat_code_cli.config import get_env_value, save_env_value
    ...
    from virat_code_cli.models import model_ids
    ...
    if selected:
        ...
        from virat_code_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from virat_code_cli.codex_models import get_codex_model_ids
    from virat_code_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from virat_code_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from virat_code_cli.auth import _save_model_choice, deactivate_provider
    from virat_code_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print("Endpoint saved. Use `/model` in chat or `virat-code model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from virat_code_cli.auth import _save_model_choice, deactivate_provider
    from virat_code_cli.config import save_env_value, load_config, save_config
    from virat_code_cli.models import fetch_api_models
...
def cmd_logout(args):
    ...
    from virat_code_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from virat_code_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from virat_code_cli.cron import cron_command
    cron_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from virat_code_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from virat_code_cli.config import config_command
    config_command(args)
...
def cmd_update(args):
    ...
    try:
        ...
        from virat_code_cli.config import (
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'Virat-Code config migrate' later to configure.")
        else:
        ...
        try:
            check = subprocess.run(
                ["systemctl", "--user", "is-active", "virat-code-gateway"],
            ...
            if check.stdout.strip() == "active":
                ...
                restart = subprocess.run(
                    ["systemctl", "--user", "restart", "virat-code-gateway"],
                ...
                if restart.returncode == 0:
                    ...
                    print("  Try manually: Virat-Code gateway restart")
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass  # No systemd (macOS, WSL1, etc.) — skip silently
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  Virat-Code model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``virat-code -c Pokemon Agent Dev`` without quoting the
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    Virat-Code                        Start interactive chat
    Virat-Code chat -q "Hello"        Single query mode
    Virat-Code -c                     Resume the most recent session
    Virat-Code -c "my project"        Resume a session by name (latest in lineage)
    Virat-Code --resume <session_id>  Resume a specific session by ID
    Virat-Code setup                  Run setup wizard
    Virat-Code logout                 Clear stored authentication
    Virat-Code model                  Select default model
    Virat-Code config                 View configuration
    Virat-Code config edit            Edit config in $EDITOR
    Virat-Code config set model gpt-4 Set a config value
    Virat-Code gateway                Run messaging gateway
    Virat-Code -w                     Start in isolated git worktree
    Virat-Code gateway install        Install as system service
    Virat-Code sessions list          List past sessions
    Virat-Code sessions browse        Interactive session picker
    Virat-Code sessions rename ID T   Rename/title a session
    Virat-Code update                 Update to latest version

For more help on a command:
    Virat-Code <command> --help
"""
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: virat-code-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    def cmd_pairing(args):
        from virat_code_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        from virat_code_cli.skills_hub import skills_command
        skills_command(args)
    ...
    def cmd_tools(args):
        from virat_code_cli.tools_config import tools_command
        tools_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch Virat-Code --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            virat_code_bin = shutil.which("Virat-Code")
            if virat_code_bin:
                os.execvp(virat_code_bin, ["Virat-Code", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "virat_code_cli.main", "--resume", selected_id],
                )
    ...
    # e.g. ``virat-code -c Pokemon Agent Dev`` → ``virat-code -c 'Pokemon Agent Dev'``
