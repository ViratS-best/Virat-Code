"""
Gateway subcommand for Virat-Code CLI.

Handles: Virat-Code gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from virat_code_cli.setup import (
...
def get_launchd_plist_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / "ai.virat-code.gateway.plist"
...
def get_virat_code_cli_path() -> str:
    """Get the path to the Virat-Code CLI."""
    # Check if installed via pip
    import shutil
    virat_code_bin = shutil.which("Virat-Code")
    if virat_code_bin:
        return virat_code_bin
    
    # Fallback to direct module execution
    return f"{get_python_path()} -m virat_code_cli.main"
...
def generate_systemd_unit() -> str:
    ...
    virat_code_cli = shutil.which("Virat-Code") or f"{python_path} -m virat_code_cli.main"
    return f"""[Unit]
...
[Service]
Type=simple
ExecStart={python_path} -m virat_code_cli.main gateway run --replace
ExecStop={virat_code_cli} gateway stop
...
def systemd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print(f"  Virat-Code gateway start              # Start the service")
    print(f"  Virat-Code gateway status             # Check status")
...
def systemd_status(deep: bool = False):
    ...
    if not unit_path.exists():
        ...
        print("  Run: Virat-Code gateway install")
        return
    ...
    if status == "active":
        ...
        print("  Run: Virat-Code gateway start")
...
def generate_launchd_plist() -> str:
    python_path = get_python_path()
    working_dir = str(PROJECT_ROOT)
    log_dir = Path.home() / ".virat-code" / "logs"
...
<dict>
    <key>Label</key>
    <string>ai.virat-code.gateway</string>
    
    <key>ProgramArguments</key>
    <array>
        <string>{python_path}</string>
        <string>-m</string>
        <string>virat_code_cli.main</string>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  Virat-Code gateway status             # Check status")
    print("  tail -f ~/.virat-code/logs/gateway.log  # View logs")
...
def launchd_start():
    subprocess.run(["launchctl", "start", "ai.virat-code.gateway"], check=True)
    print("✓ Service started")

def launchd_stop():
    subprocess.run(["launchctl", "stop", "ai.virat-code.gateway"], check=True)
    print("✓ Service stopped")
...
def launchd_status(deep: bool = False):
    result = subprocess.run(
        ["launchctl", "list", "ai.virat-code.gateway"],
        capture_output=True,
        text=True
    )
    ...
    if deep:
        log_file = Path.home() / ".virat-code" / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = Path.home() / ".virat-code" / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'virat-code pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: Virat-Code pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'Virat-Code gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from virat_code_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"Virat CodeAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: Virat-Code gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: Virat-Code gateway")
    else:
        print()
        print_info("No platforms configured. Run 'Virat-Code gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: Virat-Code gateway run")
            sys.exit(1)
        ...
        if is_linux() and get_systemd_unit_path().exists():
            ...
            if pids:
                ...
                print("  Virat-Code gateway install")
            else:
                print("✗ Gateway is not running")
                print()
                print("To start:")
                print("  Virat-Code gateway          # Run in foreground")
                print("  Virat-Code gateway install  # Install as service")
