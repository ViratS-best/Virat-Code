...
from virat_code_cli.config import (
    get_virat_code_home, get_config_path, get_env_path,
    load_config, save_config, save_env_value, get_env_value,
    ensure_virat_code_home, DEFAULT_CONFIG
)
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with 'Virat-Code setup')")


def _print_setup_summary(config: dict, virat_code_home):
    ...
    if disabled_tools:
        print_warning("Some tools are disabled. Run 'Virat-Code setup tools' to configure them,")
        print_warning("or edit ~/.virat-code/.env directly to add the missing API keys.")
        print()
    ...
    print(color("📁 All your files are in ~/.virat-code/:", Colors.CYAN, Colors.BOLD))
    print()
    print(f"   {color('Settings:', Colors.YELLOW)}  {get_config_path()}")
    print(f"   {color('API Keys:', Colors.YELLOW)}  {get_env_path()}")
    print(f"   {color('Data:', Colors.YELLOW)}      {virat_code_home}/cron/, sessions/, logs/")
    print()
    ...
    print()
    print(f"   {color('Virat-Code setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('Virat-Code setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('Virat-Code setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('Virat-Code setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('Virat-Code setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('Virat-Code config', Colors.GREEN)}         View current settings")
    print(f"   {color('Virat-Code config edit', Colors.GREEN)}    Open config in your editor")
    print(f"   {color('Virat-Code config set KEY VALUE', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('Virat-Code', Colors.GREEN)}              Start chatting")
    print(f"   {color('Virat-Code gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('virat-code doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `virat-code tools` and `Virat-Code setup tools`).
...
def setup_model_provider(config: dict):
    ...
    if provider_idx == 0:  # Nous Portal API Key (direct)
        ...
        try:
            from virat_code_cli.auth import _login_nous, ProviderConfig
            import argparse
            ...
            print_info("You can try again later with: Virat-Code model")
        ...
        try:
            from virat_code_cli.auth import deactivate_provider
            deactivate_provider()
        except Exception:
            pass
        import yaml
        config_path = Path(os.environ.get("VIRAT_CODE_HOME", Path.home() / ".virat-code")) / "config.yaml"
        try:
            disk_cfg = {}
        ...
        if api_key:
            ...
            from virat_code_cli.auth import detect_zai_endpoint
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from virat_code_cli.models import model_ids, menu_labels
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value('VIRAT_CODE_MAX_ITERATIONS') or str(config.get('agent', {}).get('max_turns', 90))
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("VIRAT_CODE_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'Virat-Code whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'Virat-Code whatsapp' to choose your mode (separate bot number")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(f"     Virat-Code config set {plat.upper()}_HOME_CHANNEL <channel_id>")
        ...
        from virat_code_cli.gateway import (
        ...
        if service_running:
            ...
            if prompt_yes_no(f"  Install the gateway as a {svc_name} service? (runs in background, starts on boot)", True):
                try:
                    ...
                    print_info("  You can try manually: Virat-Code gateway install")
            else:
                print_info("  You can install later: Virat-Code gateway install")
                print_info("  Or run in foreground:  Virat-Code gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   Virat-Code gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `Virat-Code setup tools` and `virat-code tools` use the same flow:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      Virat-Code setup           — full or quick (auto-detected)
      Virat-Code setup model     — just model/provider
      Virat-Code setup terminal  — just terminal backend
      Virat-Code setup gateway   — just messaging platforms
      Virat-Code setup tools     — just tool configuration
      Virat-Code setup agent     — just agent settings
    """
    ...
    from virat_code_cli.auth import get_active_provider
    ...
    if is_existing:
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, virat_code_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'Virat-Code setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'Virat-Code setup' again when ready.")
            return
            ...
            func(config)
            save_config(config)
            _print_setup_summary(config, virat_code_home)
            return
    else:
        # ── First-Time Setup ──
        print()
    ...
    print_info(f"Data folder:  {virat_code_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'Virat-Code config edit'")
...
def _run_quick_setup(config: dict, virat_code_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'Virat-Code setup' and choose 'Full Setup' to reconfigure,")
