...
- Virat-Code config          - Show current configuration
- Virat-Code config edit     - Open config in editor
- Virat-Code config set      - Set a specific value
- Virat-Code config wizard   - Re-run setup wizard
"""
...
def get_config_path() -> Path:
    ...
    return get_virat_code_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_virat_code_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def ensure_virat_code_home():
    """Ensure ~/.virat-code directory structure exists."""
    home = get_virat_code_home()
...
DEFAULT_CONFIG = {
    ...
    # This section is only needed for virat-code-specific overrides; everything else
...
OPTIONAL_ENV_VARS = {
    ...
    # VIRAT_CODE_TOOL_PROGRESS and VIRAT_CODE_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "VIRAT_CODE_TOOL_PROGRESS": {
    ...
    "VIRAT_CODE_PREFILL_MESSAGES_FILE": {
    ...
    "VIRAT_CODE_EPHEMERAL_SYSTEM_PROMPT": {
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("VIRAT_CODE_TOOL_PROGRESS")
            old_mode = get_env_value("VIRAT_CODE_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from VIRAT_CODE_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from VIRAT_CODE_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("VIRAT_CODE_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from VIRAT_CODE_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: Virat-Code config set KEY VALUE")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.virat-code/config.yaml."""
    import copy
...
#   openai-codex (OAuth — Virat-Code login) — OpenAI Codex
#   nous         (OAuth — Virat-Code login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.virat-code/config.yaml."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.virat-code/.env."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.virat-code/.env."""
    ensure_virat_code_home()
    env_path = get_env_path()
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.virat-code/.env or environment."""
...
def show_config():
    ...
    print(color("  Virat-Code config edit     # Edit config file", Colors.DIM))
    print(color("  Virat-Code config set KEY VALUE", Colors.DIM))
    print(color("  Virat-Code setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or not value:
            print("Usage: Virat-Code config set KEY VALUE")
            print()
            print("Examples:")
            print("  Virat-Code config set model anthropic/claude-sonnet-4")
            print("  Virat-Code config set terminal.backend docker")
            print("  Virat-Code config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print(f"    Run 'Virat-Code config migrate' to add them")
        ...
        print("  Virat-Code config           Show current configuration")
        print("  Virat-Code config edit      Open config in editor")
        print("  Virat-Code config set K V   Set a config value")
        print("  Virat-Code config check     Check for missing/outdated config")
        print("  Virat-Code config migrate   Update config with new options")
        print("  Virat-Code config path      Show config file path")
        print("  Virat-Code config env-path  Show .env file path")
        sys.exit(1)
