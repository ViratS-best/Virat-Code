...
Pure display functions with no Virat CodeCLI state dependency.
"""
...
from virat_code_cli import __version__ as VERSION
...
def check_for_updates() -> Optional[int]:
    ...
    ``~/.virat-code/.update_check``).  Returns the number of commits behind,
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    left_lines = ["", VIRAT_CODE_CADUCEUS, ""]
    model_short = model.split("/")[-1] if "/" in model else model
    if len(model_short) > 28:
        model_short = model_short[:25] + "..."
    ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Virat Sisodiya[/]")
    ...
    try:
        ...
        if behind and behind > 0:
            ...
            right_lines.append(
                ...
                f"[dim yellow] — run [bold]Virat-Code update[/bold] to update[/]"
            )
    except Exception:
    ...
    console.print()
    console.print(VIRAT_CODE_AGENT_LOGO)
    console.print()
