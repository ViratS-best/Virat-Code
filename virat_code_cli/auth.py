...
is persisted in ~/.virat-code/auth.json with cross-process file locking.
...
from virat_code_cli.config import get_virat_code_home, get_config_path
...
DEFAULT_NOUS_CLIENT_ID = "virat-code-cli"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `virat-code model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("VIRAT_CODE_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.virat-code/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_virat_code_home() / "auth.json"
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `virat-code logout`.
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `virat-code login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `virat-code login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `virat-code login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `virat-code login` to re-authenticate.",
            provider="openai-codex",
...
def resolve_codex_runtime_credentials(
    *,
    ...
    refresh_timeout_seconds = float(os.getenv("VIRAT_CODE_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = (
        os.getenv("VIRAT_CODE_CODEX_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("VIRAT_CODE_CA_BUNDLE")
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("VIRAT_CODE_PORTAL_BASE_URL")
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `virat-code status` output."""
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `virat-code login` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    from virat_code_cli.config import save_config, load_config, save_env_value
...
def login_command(args) -> None:
    """Deprecated: use 'virat-code model' or 'Virat-Code setup' instead."""
    print("The 'virat-code login' command has been removed.")
    print("Use 'virat-code model' to select a provider and model,")
    print("or 'Virat-Code setup' for full interactive setup.")
    raise SystemExit(0)
...
def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    ...
    if cli_tokens:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("VIRAT_CODE_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
    ...
    print()
    print("Login successful!")
    print(f"  Auth state: ~/.virat-code/auth.json")
    print(f"  Config updated: {config_path} (model.provider=openai-codex)")
