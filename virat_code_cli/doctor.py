...
from virat_code_cli.config import get_project_root, get_virat_code_home, get_env_path

PROJECT_ROOT = get_project_root()
VIRAT_CODE_HOME = get_virat_code_home()

# Load environment variables from ~/.virat-code/.env so API key checks work
...
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(VIRAT_CODE_HOME))
os.environ.setdefault("MSWEA_SILENT_STARTUP", "1")

from virat_code_cli.colors import Colors, color
from virat_code_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.virat-code/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # Check ~/.virat-code/.env (primary location for user config)
    env_path = VIRAT_CODE_HOME / '.env'
    if env_path.exists():
        check_ok("~/.virat-code/.env file exists")
        ...
        if _has_provider_env_config(content):
            ...
            check_warn("No API key found in ~/.virat-code/.env")
            issues.append("Run 'Virat-Code setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            check_fail("~/.virat-code/.env file missing")
            if should_fix:
                env_path.parent.mkdir(parents=True, exist_ok=True)
                env_path.touch()
                check_ok("Created empty ~/.virat-code/.env")
                check_info("Run 'Virat-Code setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'Virat-Code setup' to create one")
                issues.append("Run 'Virat-Code setup' to create .env")
    
    # Check ~/.virat-code/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = VIRAT_CODE_HOME / 'config.yaml'
    if config_path.exists():
        check_ok("~/.virat-code/config.yaml exists")
    else:
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok("Created ~/.virat-code/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append("Create ~/.virat-code/config.yaml manually")
            else:
    ...
    try:
        from virat_code_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    virat_code_home = VIRAT_CODE_HOME
    if virat_code_home.exists():
        check_ok("~/.virat-code directory exists")
    else:
        if should_fix:
            virat_code_home.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.virat-code directory")
            fixed_count += 1
        else:
            check_warn("~/.virat-code not found", "(will be created on first use)")
    
    # Check expected subdirectories
    expected_subdirs = ["cron", "sessions", "logs", "skills", "memories"]
    for subdir_name in expected_subdirs:
        subdir_path = virat_code_home / subdir_name
        if subdir_path.exists():
            check_ok(f"~/.virat-code/{subdir_name}/ exists")
        else:
            if should_fix:
                subdir_path.mkdir(parents=True, exist_ok=True)
                check_ok(f"Created ~/.virat-code/{subdir_name}/")
                fixed_count += 1
            else:
                check_warn(f"~/.virat-code/{subdir_name}/ not found", "(will be created on first use)")
    
    # Check for SOUL.md persona file
    soul_path = virat_code_home / "SOUL.md"
    if soul_path.exists():
        content = soul_path.read_text(encoding="utf-8").strip()
        # Check if it's just the template comments (no real content)
        lines = [l for l in content.splitlines() if l.strip() and not l.strip().startswith(("<!--", "-->", "#"))]
        if lines:
            check_ok("~/.virat-code/SOUL.md exists (persona configured)")
        else:
    ...
    memories_dir = virat_code_home / "memories"
    if memories_dir.exists():
        check_ok("~/.virat-code/memories/ directory exists")
        ...
        check_warn("~/.virat-code/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.virat-code/memories/")
            fixed_count += 1
    
    # Check SQLite session store
    state_db_path = virat_code_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"~/.virat-code/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"~/.virat-code/state.db exists but has issues: {e}")
    else:
        check_info("~/.virat-code/state.db not created yet (will be created on first session)")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'Virat-Code setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = VIRAT_CODE_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: Virat-Code skills list)")

    from virat_code_cli.config import get_env_value
    github_token = get_env_value("GITHUB_TOKEN") or get_env_value("GH_TOKEN")
    if github_token:
        check_ok("GitHub token configured (authenticated API access)")
    else:
        check_warn("No GITHUB_TOKEN", "(60 req/hr rate limit — set in ~/.virat-code/.env for better rates)")
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'virat-code doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
