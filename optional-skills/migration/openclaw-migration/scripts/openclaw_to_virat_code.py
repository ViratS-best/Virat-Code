...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.virat-code/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.virat-code/tts/.",
    },
    "discord-settings": {
...
class Migrator:
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        virat_code_config = load_yaml_file(destination)
        current_model = virat_code_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            virat_code_config["model"] = model_str
            dump_yaml_file(destination, virat_code_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        virat_code_config = load_yaml_file(destination)
        existing_tts = virat_code_config.get("tts", {})
        ...
        if self.execute:
            ...
            virat_code_config["tts"] = merged_tts
            dump_yaml_file(destination, virat_code_config)
