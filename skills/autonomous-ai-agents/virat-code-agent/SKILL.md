...
| Feature | `delegate_task` | Spawning `Virat-Code` process |
|---------|-----------------|--------------------------|
...
- `Virat-Code` CLI installed and on PATH
- API key configured in `~/.virat-code/.env`
...
This installs uv, Python 3.11, clones the repo, sets up the venv, and launches an interactive setup wizard to configure your API provider and model. See the [GitHub repo](https://github.com/ViratS-best/Virat-Code) for details.
...
terminal(command="virat-code --continue", background=true, pty=true)

# Resume a specific session by ID (shown on exit)
terminal(command="virat-code --resume 20260225_143052_a1b2c3", background=true, pty=true)
```
...
```
terminal(command="virat-code chat -q 'Research the latest GRPO training papers and write a summary to ~/research/grpo.md'", timeout=300)
```

Background for long tasks:
```
terminal(command="virat-code chat -q 'Set up CI/CD for ~/myapp'", background=true)
...
terminal(command="Virat-Code", workdir="~/project", background=true, pty=true)
# Returns session_id
...
**Code review loop** — spawn virat-code, send code for review, iterate on feedback:
```
terminal(command="Virat-Code", workdir="~/project", background=true, pty=true)
...
```
terminal(command="Virat-Code", background=true, pty=true)
...
```
# Agent A: backend
terminal(command="Virat-Code", workdir="~/project/backend", background=true, pty=true)
process(action="submit", session_id="<agent-a>", data="Build a REST API for user management with CRUD endpoints")

# Agent B: frontend
terminal(command="Virat-Code", workdir="~/project/frontend", background=true, pty=true)
...
```
terminal(command="virat-code chat -q 'Research competitor landing pages and write a report to ~/research/competitors.md'", background=true)
terminal(command="virat-code chat -q 'Audit security of ~/myapp and write findings to ~/myapp/SECURITY_AUDIT.md'", background=true)
process(action="list")
```
...
```
terminal(command="virat-code chat -q 'Summarize this codebase' --model google/gemini-2.5-pro", workdir="~/project", background=true)
```
...
```
# Start Virat-Code inside tmux
tmux new-session -d -s virat-code-session -x 120 -y 40 "Virat-Code"
sleep 10  # Wait for banner/startup

# Send messages
tmux send-keys -t virat-code-session "your message here" Enter

# Read output
sleep 15  # Wait for LLM response
tmux capture-pane -t virat-code-session -p

# Multi-turn: just send more messages and capture again
tmux send-keys -t virat-code-session "follow-up message" Enter

# Exit when done
tmux send-keys -t virat-code-session "/exit" Enter
tmux kill-session -t virat-code-session
```
