...
description: Build, test, and debug Virat Code RL environments for Atropos training. Covers the HermesAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the Virat Code repo.
version: 1.1.0
author: Virat Code
license: MIT
metadata:
...
2. **evaluate() must use HermesAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of Virat Code benchmarks is agentic evaluation with tool use.
