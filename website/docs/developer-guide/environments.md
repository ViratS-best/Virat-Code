...
parser = get_parser("Virat-Code")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `Virat-Code`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "virat-code[yc-bench]"
...
```bash
python environments/virat_code_swe_env/virat_code_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"Virat-Code"`, `"qwen"`, `"mistral"`)
...
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which Virat-Code toolsets to enable |
...
| `tool_call_parser` | `str` | `"Virat-Code"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "virat-code[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "virat-code[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
├── virat_code_swe_env/             # SWE-bench training environment
│
