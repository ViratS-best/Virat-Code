...
The Virat Code layer (`environments/hermes_base_env.py`). Adds:
...
`HermesAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as Virat Code's main loop:
