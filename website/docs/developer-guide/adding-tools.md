...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Virat-Code — schemas, handlers, registration, and toolsets"
---
...
2. **`toolsets.py`** — add tool name to `_VIRAT_CODE_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `virat_code_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `virat_code_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `virat-code chat -q "Use the weather tool for London"`
