...
Forgot something from your last session? Run `virat-code -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `virat-code -r "my research project"`.
...
Want the agent to be more concise? More technical? Place a `SOUL.md` in your project root or `~/.virat-code/SOUL.md` for global personality customization. This shapes the agent's tone and communication style.
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `virat-code sessions list` and resume with `virat-code -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `virat-code pairing approve telegram XKGH5N7P` — simple and secure.
...
:::tip
On messaging platforms, sessions auto-reset after idle time (default: 120 min) or daily at 4 AM. Adjust per-platform in `~/.virat-code/gateway.json` if you need longer sessions.
:::
...
TERMINAL_DOCKER_IMAGE=virat-code-sandbox:latest
```
