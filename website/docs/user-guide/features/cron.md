...
Virat-Code gateway             # Or run in foreground

virat-code cron list           # View scheduled jobs
virat-code cron status         # Check if gateway is running
```
...
1. It loads all jobs from `~/.virat-code/cron/jobs.json`
...
A **file-based lock** (`~/.virat-code/cron/.tick.lock`) prevents duplicate execution if multiple processes overlap (e.g., gateway + manual tick).
...
| `"local"` | Save to local files only (`~/.virat-code/cron/output/`) | Default on CLI |
...
virat-code cron status         # Check if the scheduler is running
...
Jobs are stored as JSON in `~/.virat-code/cron/jobs.json`. Output from job runs is saved to `~/.virat-code/cron/output/{job_id}/{timestamp}.md`.
