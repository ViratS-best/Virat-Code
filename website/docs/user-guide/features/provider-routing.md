...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Virat Code supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.virat-code/config.yaml`:
...
- **CLI mode** — configured in `~/.virat-code/config.yaml`, loaded at startup
