...
```bash
virat-code --checkpoints
```

### Permanently (config.yaml)

```yaml
# ~/.virat-code/config.yaml
checkpoints:
  enabled: true
...
Yes! Checkpoints persist in `~/.virat-code/checkpoints/` and survive across sessions. You can roll back to a checkpoint from yesterday.
