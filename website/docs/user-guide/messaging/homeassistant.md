...
description: Control your smart home with Virat-Code via Home Assistant integration.
...
Virat-Code integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Virat-Code"
5. Copy the token
...
Virat-Code registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Virat-Code".
